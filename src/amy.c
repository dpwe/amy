    ...
    if(type>2) {
        ...
        for(uint16_t i=0;i<AMY_OSCS;i++) {
            ...
            if(type>3) { 
                ...
                fprintf(stderr,"mod osc %d: amp: %f, freq %f duty %f filter_freq %f resonance %f fb/bw %f pan %f \n", i, msynth[i].amp, msynth[i].freq, msynth[i].duty, msynth[i].filter_freq, msynth[i].resonance, S2F(msynth[i].feedback), msynth[i].pan);
            }
        }
    }
}
...
void hold_and_modify(uint16_t osc) {
    ...
    for(uint8_t i=0;i<MAX_BREAKPOINT_SETS;i++) {
        ...
        if(synth[osc].breakpoint_target[i] & TARGET_AMP) msynth[osc].amp = msynth[osc].amp * S2F(scale);
        ...
        if(synth[osc].breakpoint_target[i] & TARGET_FEEDBACK) msynth[osc].feedback = msynth[osc].feedback * S2F(scale);
        if(synth[osc].breakpoint_target[i] & TARGET_FILTER_FREQ) msynth[osc].filter_freq = msynth[osc].filter_freq * (1.0f + S2F(scale));   // Env only increases base filter freq.
    ...
    if(synth[osc].mod_target & TARGET_AMP) msynth[osc].amp = msynth[osc].amp + msynth[osc].amp * S2F(scale);
    ...
    if(synth[osc].mod_target & TARGET_FEEDBACK) msynth[osc].feedback = msynth[osc].feedback + (msynth[osc].feedback * S2F(scale));
...
void parse_control_coefs(char *message, float *control_coefs) {
    // Controls are formed as linear sum of 6 sources, each with its own
    // coupling coefficients.  These are specified in the interface with
    // lists of up to 6 real coefficients, applied, respectively, to
    // constant(1.0), note pitch, note velocity, env 1, env 2, mod_osc.
    // Unspecified value are set to 0.
    int num_vals_set = parse_float_list_message(message, control_coefs,
                                                NUM_CONTROL_INPUTS);
    // Set unspecified values to zero.
    for (int i = num_vals_set; i < NUM_CONTROL_INPUTS; ++i)
        control_coefs[i] = 0.0;
}
